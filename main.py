...
def main():
    ...
    while retry_count <= max_retries:
        try:
            logger.info(
                f"Attempt {retry_count + 1}/{max_retries + 1}: Running extraction_sub()")
            extraction_sub()
            logger.info("Successfully completed extraction_sub()")
            break
        except Exception as e:
            retry_count += 1
            logger.error(f"Error in extraction_sub: {str(e)}")
