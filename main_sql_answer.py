...
def main():
    ...
    while retry_count <= max_retries:
        try:
            logger.info(
                f"Attempt {retry_count + 1}/{max_retries + 1}: Running sql_answer_sub()")
            sql_answer_sub()
            logger.info("Successfully completed sql_answer_sub()")
            break
        except Exception as e:
            retry_count += 1
            logger.error(f"Error in sql_answer_sub: {str(e)}")
